...
import com.salesforce.automation.SalesforceEngageAutomation.*;
...
public class LeadCreation_FormulaFields extends RenewalsTestBaseClass {

    private static final Logger LOG = LoggerFactory.getLogger(LeadCreation_FormulaFields.class);
    ...
    public static String productNameThree = System.getProperty("productNameThree");
    ...
    public static String Rmbrand = System.getProperty("Rmbrand");
    public static String RenewalOpptyOwnerName1 = System.getProperty("RenewalOpptyOwnerName1");;
    ...
    public void LeadCreation_FormulaFields(){

        System.setProperty("jobName", "LeadCreation_FormulaFields");
        ...
        try {
            ...
            EngageDataCreation engageTest =  new EngageDataCreation(driver, 360);

            String LeadName="Test1";


            engageTest.createLead(connection, LeadName);
            engageTest.createContact(connection, LeadName);
...
/*
          ...
          /*  commonAPI.updateRecordStatus(connection, "Contract", contractId, "Activated");
            ...
            // Create New Order
            /*This Method is mapped to Taleggio-ID   T-2895033*/
          /*  String orderId = orderAPI.createOrder(connection, contractId, ContractStartDate, 12, priceBookId,
            ...
            String PricebookEntryId_Query_Three = "SELECT Id, Pricebook2Id, Product2.name, Product2.Id"
                    + " FROM PricebookEntry WHERE Pricebook2.Name = '" + priceBookName + "'"
                    + " AND Product2.Quotable__c = 'Yes' AND Product2.sfbase__Status__c = 'Active'"
                    + " AND Product2.IsActive = true AND unitprice >0 AND PricebookEntry.BillingFrequency=1"
                    + " AND Product2.name like '" + productNameThree + "'";
            SObject[] SObject_PricebookEntryId_Query_Three = ApiUtilities.executeQuery(connection, PricebookEntryId_Query_Two);
            String PricebookEntryId_Three = (String) SObject_PricebookEntryId_Query_Three[0].getField("Id");

            // Add Order Line Item to Order
            orderAPI.createOrderLineItem(connection, orderId, 10, PricebookEntryId_Three);
        ...
        /*    commonAPI.updateRecordStatus(connection, "Order", orderId, "Provisioned");
            ...
            // Create COSO
            /*This Method is mapped to Taleggio-ID T-3046476*/
       /*     contractAPI.createCOSO(connection, contractId);
            ...
            String RMBrand1 = (String) SObject_RMBrand[0].getField("Rm_Brand__c");
            System.out.println(Rmbrand );
            System.out.println(RMBrand1);
            Assert.assertEquals(Rmbrand, RMBrand1);
            LOG.info(" : Renewal Oppty: Create Renewal Oppty - Start");
            //Adding UI Call instead of APICall.

            /**
             This validation part of RM Brand  Regression Testing
             T-3046474: COSO Total On Contract after COSO creation
             */
        ...
        /*    String RenewalOpptyId = opportunityAPI.CreateRenewalOppty(connection, contractId);
            ...
            String RenewalOpptyOwnerIdQuery = "SELECT OwnerId FROM Opportunity where Id='" + RenewalOpptyId + "'";

            SObject[] SObject_RenewalOpptyOwnerId = ApiUtilities.executeQuery(connection, RenewalOpptyOwnerIdQuery);

            String RenewalOpptyOwnerId = (String) SObject_RenewalOpptyOwnerId[0].getField("OwnerId");

            String RenewalOpptyOwnerName = commonAPI.getFieldValue(connection, "User", RenewalOpptyOwnerId, "Name");
            LOG.info(" : Changing Renewal Oppty Owner as " + RenewalOpptyOwnerName);
            commonAPI.updateField(connection, "Opportunity", RenewalOpptyId, "OwnerId", RenewalOpptyOwnerId);

            System.out.println(RenewalOpptyOwnerName);
            System.out.println (RenewalOpptyOwnerName1);
            Assert.assertEquals(RenewalOpptyOwnerName, RenewalOpptyOwnerName1);
            ...
            driver.close();
           // Assert.assertNull("Renewal Oppty Failed");
        }
    }
    ...
    /**
     This validation part of RM Brand  Regression Testing
     T-3108089: COSO Total On Contract after COSO creation
     */
    ...
    public static void calculationHybridAmount(PartnerConnection connection, String ContractId){
        try {
            ...
            LOG.info("Rmbrand Value =: " + Rmbrand);
            LOG.info("RenewalOpptyOwnerName Value =: " + RenewalOpptyOwnerName1);
